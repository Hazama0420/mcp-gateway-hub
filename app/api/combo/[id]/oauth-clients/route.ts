// app/api/combo/[id]/oauth-clients/route.ts
import { NextResponse } from 'next/server';
import { getServerSession } from 'next-auth';
import { authOptions } from '@/app/api/auth/[...nextauth]/route';
import prisma from '@/lib/prisma';
import { listComboOAuthClients, createComboOAuthClient } from '@/lib/oauth/store';
...
export async function GET(
  ...
  const clients = await listComboOAuthClients(comboId, user.id);
...
export async function POST(
  ...
  try {
    ...
    const newClient = await createComboOAuthClient({
      comboId,
      userId: user.id,
    ...
    recordSecurityEvent({
      ...
      endpointId: comboId,
      userId: user.id,
      route: `/api/combo/${comboId}/oauth-clients`,
      metadata: {
    ...
    return NextResponse.json(
      { error: 'Failed to create OAuth client for Combo', message: error.message },
      { status: 400 }
    );
  }
}
