// components/combo/edit-combo-modal.tsx
'use client';
  ...
  React.useEffect(() => {
    if (open && combo) {
      ...
      if (Array.isArray(combo.endpoints)) {
        combo.endpoints.forEach((link: any) => {
          if (link.endpoint_id) activeIds.add(link.endpoint_id);
          else if (link.endpoint?.id) activeIds.add(link.endpoint.id);
        });
      }
      setSelectedEndpointIds(activeIds);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedEndpointIds.size === 0) {
        throw new Error('A Combo must contain at least one adapter.');
      }
      ...
      setSuccessMessage(`Combo updated successfully (${data.tool_count || activeTools.length} tools active).`);
      setTimeout(() => {
        onSuccess();
        onOpenChange(false);
      }, 1000);
    } catch (err: any) {
      setErrorMessage(err.message || 'Error updating combo');
    } finally {
      ...
      <DialogContent className="sm:max-w-[740px] max-h-[92vh] overflow-y-auto bg-[var(--color-surface)] border-2 border-[var(--color-border)] shadow-[6px_6px_0px_0px_rgba(15,23,42,1)] rounded-2xl text-[var(--color-text-primary)] p-0 gap-0">
        ...
        <div className="p-6 pb-4 border-b-2 border-[var(--color-border)] bg-[var(--color-surface-elevated)] space-y-2">
          ...
          <p className="text-xs text-[var(--color-text-secondary)] font-medium">
            Update name, description, or attached adapters. Tools and URL will automatically stay in sync.
          </p>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 pt-2 border-t-2 border-[var(--color-border)]">
              <div className="flex items-center justify-between">
                <Label className="text-xs font-mono font-black uppercase text-[var(--color-text-primary)]">
                  Attached Adapters <span className="text-rose-500">*</span>
                </Label>
              ...
              <div className="max-h-36 overflow-y-auto p-3 rounded-xl bg-[var(--color-surface-elevated)] border-2 border-[var(--color-border)] space-y-1.5 font-mono text-[11px]">
                {activeTools.length === 0 ? (
                  <p className="text-[var(--color-text-muted)] italic">
                    No tools active. Please select at least one adapter above.
                  </p>
                ) : (
        ...
        <div className="p-4 border-t-2 border-[var(--color-border)] bg-[var(--color-surface-elevated)] flex items-center justify-end gap-3">
          ...
          >
            ...
            <span>{isSubmitting ? 'Saving Changes...' : 'Save Combo Changes'}</span>
          </Button>
        </div>
      </DialogContent>
    </Dialog>
